    ...
    <div class="card-body">
      <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-2 gap-sm-3">
        <div class="d-flex align-items-center">
          ...
          <select v-model="localOrder" @change="emit('changeSort', localOrder)" class="form-select form-select-sm flex-grow-1">
            <option value="newest">Newest First</option>
            <option value="oldest">Oldest First</option>
          </select>
        </div>
        <button @click="emit('loadMore')" class="btn btn-sm btn-outline-primary ms-sm-auto">
